...
public class EntityAIWorkKeepItemInHandAndResupply extends Goal
{
    ...
    public EntityAIWorkKeepItemInHandAndResupply(ZombiePlayer entityObjIn)
    {
    ...
    public void tick() {
        ...
        if (entityObj.hasNeededWorkItemInExtra()) {
            entityObj.takeUpTo1StackOfWorkItemFromContainer(entityObj.getExtraInventory());
            stop();
            return;
        }
        ...
        if (posCachedBestChest != BlockPos.ZERO) {
            ...
            if (dist <= 3D) {
                ...
                entityObj.takeUpTo1StackOfWorkItemFromChest(posCachedBestChest);
                stop();
