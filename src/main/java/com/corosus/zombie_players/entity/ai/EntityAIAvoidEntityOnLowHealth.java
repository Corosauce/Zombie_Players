...
public class EntityAIAvoidEntityOnLowHealth<T extends Entity> extends EntityAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        else
        {
            ...
            int prevDist = (int) theEntity.getMaximumHomeDistance();
            theEntity.setHomePosAndDistance(theEntity.getHomePosition(), -1);
            ...
            theEntity.setHomePosAndDistance(theEntity.getHomePosition(), prevDist);

            if (vec3d == null)
            {
                return false;
            }
            else if (this.closestLivingEntity.getDistanceSq(vec3d.x, vec3d.y, vec3d.z) < this.closestLivingEntity.getDistanceSq(this.theEntity.getPosition()))
            {
                return false;
            }
            else
    ...
    {
        if (this.theEntity.getDistanceSq(this.closestLivingEntity.getPosition()) < 49.0D)
        {
